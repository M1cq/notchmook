        ...
        VStack(alignment: .leading, spacing: 12) {
            VStack(alignment: .leading, spacing: 3) {
                Text("Appearance")
                    ...
                    .foregroundStyle(.white)
                Text("Choose the Nook color and the content layout.")
                    .font(.system(size: 12, weight: .medium))
                    .foregroundStyle(.white.opacity(0.52))
            }
                            ...
                            Button {
                                ...
                                VStack(spacing: 6) {
                                    Circle()
                                        ...
                                        .overlay(
                                            Circle()
                                                .stroke(.white.opacity(theme == .black ? 0.22 : 0.0), lineWidth: 1)
                                        )
                                        .overlay {
                                    ...
                                    Text(theme.title)
                                        ...
                                        .foregroundStyle(.white.opacity(model.theme == theme ? 0.95 : 0.52))
                                        .lineLimit(1)
                                }
                                .frame(width: 58, height: 58)
                                .background(
                                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                                        .fill(model.theme == theme ? .white.opacity(0.12) : .black.opacity(0.20))
                                )
                            }
                            .buttonStyle(.plain)
                        }
                    ...
                    HStack(spacing: 8) {
                        ForEach(NookLayout.allCases) { layout in
                            Button {
                                ...
                                .foregroundStyle(model.nookLayout == layout ? .black : .white)
                                .padding(.horizontal, 12)
                                .frame(height: 34)
                                .background(
                                    Capsule()
                                        .fill(model.nookLayout == layout ? .white.opacity(0.92) : .black.opacity(0.24))
                                )
                            }
                            .buttonStyle(.plain)
                        }
